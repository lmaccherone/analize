...
task('doctest', 'Test examples in documenation.', () ->
  ...
  runSync('coffeedoctest', ['--readme', 'src', 'analize.coffee'])
)
...
task('test', 'Run the test suite with nodeunit and record coverage.', () ->
  ...
  reporters.default.run(['test'], undefined, (failure) ->
    if failure?
      ...
      console.log('To see coverage report, run `istanbul report` and `open coverage/lcov-report/analize/src/index.html`')
  )
)
